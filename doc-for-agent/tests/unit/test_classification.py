...
class RepoClassificationTests(unittest.TestCase):
    ...
    def test_dogfooding_synthesis_prefers_rule_style_conclusions(self) -> None:
        with tempfile.TemporaryDirectory(prefix="doc-for-agent-dogfooding-synthesis-") as tmpdir:
            ...
            self.assertTrue(any("Verification gate:" in line for line in execution_confirmed))
            self.assertTrue(any("Verification order:" in line for line in execution_confirmed))
            self.assertTrue(any("stop at the first failing command" in line for line in execution_confirmed if line.startswith("Verification order:")))
            self.assertTrue(any("Failure triage priority:" in line for line in execution_confirmed))
            self.assertTrue(any("1) run `docagent doctor` first" in line for line in execution_confirmed if line.startswith("Failure triage priority:")))
            self.assertTrue(any("roll back generated docs to last known-good state" in line for line in execution_confirmed if line.startswith("Failure triage priority:")))
            self.assertTrue(any("CLI boundary:" in line for line in architecture_confirmed))
            self.assertTrue(any("Source-of-truth boundary:" in line for line in architecture_confirmed))
            self.assertTrue(any("on conflicts, arbitrate against" in line for line in architecture_confirmed if line.startswith("Source-of-truth boundary:")))
            ...
            self.assertTrue(any("Conflict handling order:" in line for line in architecture_confirmed))
            self.assertTrue(any("Distribution structure:" in line for line in architecture_confirmed))
            self.assertTrue(any("`Codex` -> `docagent init --ai codex --target <repo-root>`" in line for line in architecture_confirmed))
            self.assertTrue(any("Product positioning:" in line for line in product_confirmed))
            self.assertTrue(any("Positioning guardrail:" in line for line in product_confirmed))
