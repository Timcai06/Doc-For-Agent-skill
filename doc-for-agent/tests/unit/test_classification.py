...
class RepoClassificationTests(unittest.TestCase):
    ...
    def test_dogfooding_synthesis_prefers_rule_style_conclusions(self) -> None:
        with tempfile.TemporaryDirectory(prefix="doc-for-agent-dogfooding-synthesis-") as tmpdir:
            ...
            self.assertTrue(any("Verification gate:" in line for line in execution_confirmed))
            self.assertTrue(any("Failure triage:" in line for line in execution_confirmed))
            self.assertTrue(any("run `docagent doctor` first" in line for line in execution_confirmed if line.startswith("Failure triage:")))
            self.assertTrue(any("roll back generated docs to last known-good state" in line for line in execution_confirmed if line.startswith("Failure triage:")))
