...
- This page is maintainer-facing source-of-truth for its domain; keep it synchronized with `AGENTS/` in dual mode and `human/` as the human-view root.
...
- After edits, refresh in dual mode and verify both `AGENTS/` and `human/` were updated in the same change set.
...
- Product pairing rule: if `human/overview.md` changes due to scope/value decisions, refresh paired product paths under both AGENTS roots.
...
- Locale-output rule: human locale `en` maps to `human/`.
...
- Path pair rule: `human/overview.md` pairs with `AGENTS/01-product/001-core-goals.md` for agent-facing product rules and scope guardrails.
- Path pair rule: `human/overview.md` pairs with `AGENTS/01-product/002-prd.md` for agent-facing user and outcome contract.
- Path pair rule: `human/overview.md` pairs with `AGENTS/01-product/003-app-flow.md` for agent-facing flow and entry-surface notes.
...
- Use `human/` for maintainer-facing policy and decisions; use `AGENTS/` for execution order, command wiring, and handoff runbooks.
- If a change affects both reader types, update both systems in one dual refresh cycle instead of patching only one side.
- Keep user/value framing in `human/overview.md`; keep edit-time operating rules in `AGENTS/product.md` (or layered product docs).

## Dual View Rationale

- `human/` and `AGENTS/` are two views generated from the same repository analysis and source-of-truth anchors.
- When the two views diverge, treat it as refresh drift rather than independent documentation authority.
- Product intent lives in `human/overview.md`, while execution-facing preservation rules live in paired AGENTS product docs.
